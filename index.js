...
const REDIRECT_PAGE_TEMPLATE = [
  ...
  '    </div>',
  '',
  '    <a class="manual-link" href="__REDIRECT_HREF__">Continue now</a>',
  '',
  ...
  '        if (parsedUrl.protocol === \x27https:\x27 || parsedUrl.protocol === \x27http:\x27) {',
...
const ROBOTS_TXT = [
  '# Allow Stripe verification bots',
  'User-agent: Stripe',
  'Allow: /',
  '',
...
function generateRedirectPage(redirectUrl) {
  ...
  return replaceTemplateTokens(REDIRECT_PAGE_TEMPLATE, {
    '__REDIRECT_HREF__': safeRedirectHref,
    '__REDIRECT_URL_JSON__': JSON.stringify(safeRedirectUrl),
    '__FALLBACK_URL_JSON__': JSON.stringify(safeFallbackUrl)
  });
}
